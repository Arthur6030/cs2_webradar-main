...
bool schema::setup() {
  ...
  std::unique_ptr<uintptr_t[]> elements =
      std::make_unique_for_overwrite<uintptr_t[]>(table_size);
  ...
  for (uint32_t idx = 0; idx < elements_size; idx++) {
    ...
    for (uint32_t f_idx = 0; f_idx < schema_field_size; f_idx++) {
      ...
      auto buff = format("{}->{}", class_binding->m_binary_name(),
                         schema_field->m_name());
      m_schema_data.emplace_back(fnv1a::hash(buff),
...
uint32_t schema::get_offset(const fnv1a_t hashed_field_name) {
  if (const auto it = std::ranges::find_if(
          m_schema_data,
          [hashed_field_name](const schema_data_t &data) {
  ...
  LOG_ERROR("failed to find an offset for the field with the hash value '%d'",
            hashed_field_name);
  return {};
}
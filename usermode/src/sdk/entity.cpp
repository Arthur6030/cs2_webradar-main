...
const std::string c_cs_player_pawn::get_model_name() {
  const auto model_name = m_memory->read_t<uintptr_t>(
      m_pGameSceneNode() +
      SCHEMA_GET_OFFSET("CSkeletonInstance->m_modelState") +
      SCHEMA_GET_OFFSET("CModelState->m_ModelName"));
  if (!model_name)
    return {};
  ...
  return model_path.substr(model_path.rfind("/") + 1,
                           model_path.rfind(".") - model_path.rfind("/") - 1);
}

c_cs_player_controller *c_cs_player_controller::get_local_player_controller() {
  static auto offset =
      m_memory->find_pattern(CLIENT_DLL(), GET_LOCAL_PLAYER_CONTROLLER())
          ->rip()
          .as<void *>();
  if (!offset)
    return {};
...
c_base_player_weapon *c_base_player_weapon::get(const int32_t idx) {
  ...
  return i::m_game_entity_system->get<c_base_player_weapon *>(handle &
                                                              ENT_ENTRY_MASK);
}